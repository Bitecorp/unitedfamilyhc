<table id="tableAgents" class="table table-striped table-bordered table-td-valign-middle">
    ...
    <tbody>
        ...
        @foreach($agents['AI'] as $key => $agentI)
            <tr data-id='{{ $agentI->id }}' id="data_{{ $key }}">
                <td>{{ $agentI->first_name }} {{ $agentI->last_name }}</td>
                <td>{{ $agentI->home_phone }}</td>
                <td>{{ $agentI->email }}</td>
                @foreach($roles as $role)
                    @if($agentI->role_id == $role->id)
                        <td>{{ $role->name_role }}</td>
                    @endif
                @endforeach
                @foreach($status as $statu)
                    @if($agentI->statu_id == $statu->id)
                        @if($agentI->id == 1)
                            <td>{{ $statu->name_status }}</td>
                        @elseif($agentI->id > 1)
                            <td>
                                {!! Form::model($agentI, ['route' => ['agents.updateState', $agentI->id], 'method' => 'post', 'id' => "sendForm_$agentI->id"]) !!}
                                <!-- begin custom-switches -->
                                    <div class="custom-control custom-switch">
                                        <input type="checkbox" onclick="changeStatus('{{$agentI->id}}');"  class="custom-control-input" data-id="{{ $agentI->id }}" name="{{$agentI->id}}" id="Switch_{{$agentI->id}}" {{ $agentI->statu_id == 1 ? 'checked' : '' }}>
                                        <label class="custom-control-label" for="Switch_{{$agentI->id}}"></label>
                                    </div>
                                <!-- end custom-switches -->
                                {!! Form::close() !!}
                            </td>
                ...
                <td class="with-btn" nowrap>
                    @if($agentI->id > 1 )
                        {!! Form::open(['route' => ['agents.destroy', $agentI->id], 'method' => 'delete']) !!}
                            <a href="{{ route('agents.show', [$agentI->id]) }}" class='btn btn-sm btn-primary' ><i class="fa fa-eye"></i> Show </a>
                            @if(Auth::user()->role_id == 1)
                                {!! Form::button('<a><i class="fa fa-trash"></i> Delete </a>', ['type' => 'submit', 'class' => 'btn btn-sm btn-danger', 'onclick' => "return confirm('You are about to delete a root record, if the information is not saved all documents attached to this worker will be lost. (Valid Documents and Expired Documents, Signed Contracts, Etc.).')"]) !!}
                            @endif
                        {!! Form::close() !!}
