									...
									@if (isset($dataMensual) && !empty($dataMensual) && isset($dataMensual['dataW']) && !empty($dataMensual['dataW']))
										@foreach ($dataMensual['dataW'] as $key => $dataFullW )
											<?php 
												...
												$memo = json_decode($dataFullW->patiente_id)->first_name . ' ' . json_decode($dataFullW->patiente_id)->last_name . ' ' . $explodeIM . ' ' . $dataFullW->time_attention . ' = ' . $dataFullW->unid_pay_worker . $crediMemo . ' - from ' . date_format(date_create(data_previa_month_day_first()), 'd_m_Y') . ' to ' . date_format(date_create(data_previa_month_day_last()), 'd_m_Y');
												$valToPai = number_format((float)($dataFullW->mont_pay - $dataFullW->montMemos), 2, '.', '');
												$hiddenBtnMemo = $dataFullW->paid == true ? 'hidden' : '';
											?>
											<tr>
