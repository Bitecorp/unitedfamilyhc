									...
									@if (isset($dataMensual) && !empty($dataMensual) && isset($dataMensual['dataW']) && !empty($dataMensual['dataW']))
										@foreach ($dataMensual['dataW'] as $key => $dataFullW )
											<?php 
												...
												$memo = json_decode($dataFullW->patiente_id)->first_name . ' ' . json_decode($dataFullW->patiente_id)->last_name . ' ' . $explodeIM . ' ' . $dataFullW->time_attention . ' = ' . $dataFullW->unid_pay_worker . $crediMemo . ' - from ' . date_format(date_create(data_previa_month_day_first()), 'm_d_Y') . ' to ' . date_format(date_create(data_previa_month_day_last()), 'm_d_Y');
